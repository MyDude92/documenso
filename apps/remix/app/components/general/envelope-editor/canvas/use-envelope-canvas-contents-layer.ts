 ...
 * and the selection actions for contents.
 */
export const useEnvelopeCanvasContentsLayer = ({
  ...
  /**
   * Resolve a marquee selection box into a content selection.
   *
   * Contents cannot be created via the marquee, they are placed from the
   * palette.
   */
  const selectInBox = (box: EnvelopeCanvasBox) => {
    ...
    selection.select('content', findEnvelopeCanvasGroupsInBox(currentStage, ENVELOPE_CANVAS_GROUP_NAMES.content, box));
  };
  ...
  /**
   * Clear any active content selection when contents are no longer editable,
   * e.g. when switching back to the fields tab or hiding contents via the
   * viewer toolbar.
   */
  useEffect(() => {
    if (!isEditable && selectedGroups.length > 0) {
      selection.clear();
      pageLayer.current?.batchDraw();
    }
  }, [isEditable, selectedGroups]);

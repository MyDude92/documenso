...
export const useEnvelopeCanvasFieldsLayer = ({
  ...
  /**
   * Remove any pending field creation rectangle from the canvas.
   */
  const clearPending = () => {
    setPendingCreation(null);

    for (const node of pageLayer.current?.find(`.${PENDING_FIELD_NODE_NAME}`) ?? []) {
      node.destroy();
    }
  };
  ...
  const createFromPending = (type: FieldType) => {
    ...
    const pixelWidth = pendingCreation.width();
    const pixelHeight = pendingCreation.height();
    const pixelX = pendingCreation.x();
    const pixelY = pendingCreation.y();
   ...
   */
  useEffect(() => {
    if (fieldsVisibility !== 'visible' && (selectedGroups.length > 0 || pendingCreation)) {
      clearPending();
      selection.clear();
      pageLayer.current?.batchDraw();
    }
  }, [fieldsVisibility, selectedGroups, pendingCreation]);
